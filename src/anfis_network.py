...
class ANFISNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        sum_weights = firing_strengths.sum(dim=1, keepdim=True) + 1.0
        ...
        rule_outputs = (x_expanded * self.consequent_weights).sum(
            dim=2
    ...
    def get_rules(self):
        ...
        rules = []
        # Placeholder for rule extraction logic (mu, sigma available from self.fuzzification)
        return rules
