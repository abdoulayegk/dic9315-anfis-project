...
class FeatureSelector:
    ...
    def __init__(
        self, n_features=config.N_FEATURES_ANFIS, random_seed=config.RANDOM_SEED
    ):
    ...
    def mutual_info_selection(self, X, y):
        ...
        feature_mi = list(zip(feature_names, mi_scores, strict=False))
    ...
    def transform(self, X):
        ...
        if isinstance(X, pd.DataFrame):
            ...
            feature_indices = [
                i for i, f in enumerate(X.columns) if f in self.selected_features
            ]
